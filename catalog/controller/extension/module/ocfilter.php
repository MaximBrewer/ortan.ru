...
class ControllerExtensionModuleOCFilter extends Controller {
  ...
  public function initialise() {
    ...
    $this->document->addStyle('/catalog/view/theme/ortan/stylesheet/ocfilter/ocfilter.css');
  ...
  public function page_links(){
    ...
    $results = $this->model_catalog_ocfilter->getAllPages();

    $data['links'] = [];
    $i = 1;
    foreach ($results as $result) {
        $path = $this->getPathByCategory($result['category_id']);
    ...
    $this->response->setOutput($this->load->view('product/ocfilter_page_links.tpl', $data));
  }
  ...
  private function sort_values($a, $b){
