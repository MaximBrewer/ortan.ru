...
class ControllerExtensionPaymentPaykeeper extends Controller
{
    ...
    {
        return $this->generatePaymentForm($this->session->data['order_id']);
    }
    ...
    public function generatePaymentForm($order_id)
    {
        ...
        if (array_key_exists('shipping_method', $this->session->data)) {
            ...
            $shipping_tax_rate = 0;
            $shipping_tax_rate = $this->getRate(
            ...
            if (
                !$pk_obj->checkDeliveryIncluded($pk_obj->getShippingPrice(), $shipping_name)
        ...
        $handling_status = $this->db->query("select * from  " . DB_PREFIX . "setting where `key`='handling_status' ");
        $handling_status = $handling_status->row['value'];

        if ($handling_status) {
            $handling_fee = $this->db->query("select * from  " . DB_PREFIX . "setting where `key`='handling_fee' ");
            $handling_fee = $handling_fee->row['value'];
            $handling_tax_class_id = $this->db->query("select * from  " . DB_PREFIX . "setting where `key`='handling_tax_class_id' ");
            $handling_tax_class_id = $handling_tax_class_id->row['value'];

            $taxes = array("tax" => "none", "tax_sum" => 0);
            $taxes = $pk_obj->setTaxes($this->tax->getRates($handling_fee, $handling_tax_class_id));
            $pk_obj->updateFiscalCart(
                $pk_obj->getPaymentFormType(),
        ...
        if ($pk_obj->getPaymentFormType() == "create") { //create form
            ...
            $sum = 0;
            foreach (json_decode($fiscal_cart_encoded) as $position) $sum += $position->sum;
