  ...
  public function index()
  {
    ...
    if (isset($settings['all_status'])) {
      ...
      if (isset($this->request->get['rating'])) {
        $filter_data['filter_rating'] = $this->request->get['rating'];
        $data['rating_checked'] = explode(',', $filter_data['filter_rating']);
      } else {
        unset($filter_data['filter_rating']);
        $data['rating_checked'] = [];
      }

      $canonical = $this->url->link('product/ex_reviews_page');
      $this->document->addLink($canonical, 'canonical');

      $review_total = $this->model_catalog_ex_reviews_page_my->getTotalReviews($filter_data);

      $results = $this->model_catalog_ex_reviews_page_my->getReviews($filter_data);
      ...
      if ($results) {
        ...
        foreach ($results as $result) {
          ...
          $review_images = $this->model_catalog_ex_reviews_page_my->getImagesByReviewId($result['review_id']);
          $children_reviews = $this->model_catalog_ex_reviews_page_my->getChildrenReviews($result['review_id']);
      ...
      if (isset($settings['all_pagination']) || !isset($settings['all_show_more'])) {
        ...
        $params = "&page={page}";
        if (isset($filter_data['filter_rating'])) $params .= "&rating=" . $filter_data['filter_rating'];
        $pagination->url = $this->url->link('product/ex_reviews_page', $params);
        $data['pagination'] = $pagination->render();

        if ($review_total <= ($page - 1) * $limit) {
          $params = "&page=" . ceil($review_total / $limit);
          if (isset($filter_data['filter_rating'])) $params .= "&rating=" . $filter_data['filter_rating'];
          $this->response->redirect($this->url->link('product/ex_reviews_page', $params));
        }
      }
