		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				if (!$json) {
                    ...
                    $user_id = !empty($this->request->post['user_id']) ? $this->request->post['user_id'] : 0;
                    $is_edit_notify = isset($this->request->post['notify']) ? $this->request->post['notify'] : false;
					$this->model_checkout_order->addOrderHistory($order_id, $order_status_id, '', false, false, $user_id, $is_edit_notify);
				}
			} else {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				$this->model_checkout_order->addOrderHistory($order_id, $this->request->post['order_status_id'], $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override'], $user_id, true);
