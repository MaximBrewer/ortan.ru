...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
        ...
        $this->model_extension_module_bitrix24->push_order($order_id);
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'bitrix24_deal_id'        => $order_query->row['bitrix24_deal_id'],
	...
	public function getOrderProducts($order_id) {
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` o WHERE o.order_id = '" . (int)$order_id . "'");
		return $order_query->rows;
	}

	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $user_id = 0, $is_edit_notify = true) {
		...
		if ($order_info) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', `user_id`='" . intval($user_id) . "', date_added = NOW()");
			...
			if (!$order_info['order_status_id'] && $order_status_id && $is_edit_notify) {
	...
	public function setBitrixDealId($order_id, $deal_id){
        $q = "UPDATE `".DB_PREFIX."order` SET `bitrix24_deal_id`='".$this->db->escape($deal_id)."' WHERE `order_id`='".(int)$order_id."'";
	    $this->db->query($q);
    }
}