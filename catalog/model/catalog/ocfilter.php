...
class ModelCatalogOCFilter extends Model
{
  ...
  public function getProductPrices($data)
  {
    ...
    $sql = "SELECT MIN(p.rub_price) AS `min`, MAX(p.rub_price) AS `max` FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)";
    ...
    $sql .= " WHERE p.status = '1' AND p.rub_price > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "'";
    ...
    if ($this->config->get('ocfilter_consider_special')) {
      ...
      $sql .= " WHERE p.status = '1' AND p.rub_price > '0' AND ps.price > '0' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->db->escape(date('Y-m-d')) . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->db->escape(date('Y-m-d')) . "'))";
    ...
    if ($this->config->get('ocfilter_consider_discount')) {
      $sql = "SELECT MIN(pd.rub_price) AS `min`, MAX(pd.rub_price) AS `max` FROM " . DB_PREFIX . "product_discount pd LEFT JOIN " . DB_PREFIX . "product p ON (pd.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)";
      ...
      $sql .= " WHERE p.status = '1' AND p.rub_price > '0' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND pd.rub_price > '0' AND pd.quantity > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND pd.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((pd.date_start = '0000-00-00' OR pd.date_start < '" . $this->db->escape(date('Y-m-d')) . "') AND (pd.date_end = '0000-00-00' OR pd.date_end > '" . $this->db->escape(date('Y-m-d')) . "'))";
    ...
    if ($this->config->get('ocfilter_consider_option')) {
      $sql = "SELECT
              ...
              FROM (
                SELECT
                  COALESCE(
                    IF(pov.rub_price = '-', p.rub_price - pov.rub_price, NULL),
                    IF(pov.rub_price = '+', p.rub_price + pov.rub_price, NULL),
                    IF(pov.rub_price = '*', p.rub_price + p.rub_price * pov.rub_price, NULL),
                    IF(pov.rub_price = '%', p.rub_price + p.rub_price * (pov.rub_price / 100), NULL),
                    IF(pov.rub_price = '=', pov.rub_price, NULL),
                    p.rub_price + pov.rub_price,
                    p.rub_price) AS option_price
      ...
      $sql .= " WHERE p.status = '1' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND pov.rub_price > '0' AND pov.quantity > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
      ...
      $cache_sqlkey = 'sql.data.' . md5(serialize($sql));

      $psql_data = $this->cache->get($cache_key);

      if (false === $psql_data) {
        $query = $this->db->query($sql);
        $psql_data = $query->row;
        $this->cache->set($cache_sqlkey, $psql_data);
      }

      if ($psql_data && $psql_data['min'] > 0) {
        $price_from[] = $psql_data['min'];

        $price_to[] = $psql_data['max'];
      }
    }
    ...
    foreach ($params as $option_id => $values) {
      ...
      if ($option_id == 'p') {
        ...
        if (isset($range['from']) && isset($range['to'])) {
          ...
          $where .= " AND (p.rub_price BETWEEN '" . (float)$price_from . "' AND '" . (float)$price_to . "'";

          if ($this->config->get('ocfilter_consider_discount')) {
            $where .= " OR EXISTS (SELECT pd.product_id FROM " . DB_PREFIX . "product_discount pd WHERE pd.product_id = p.product_id AND pd.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd.quantity > '0' AND ((pd.date_start = '0000-00-00' OR pd.date_start < '" . $this->db->escape(date('Y-m-d')) . "') AND (pd.date_end = '0000-00-00' OR pd.date_end > '" . $this->db->escape(date('Y-m-d')) . "')) AND pd.rub_price BETWEEN '" . (float)$price_from . "' AND '" . (float)$price_to . "')";
          }
          ...
          if ($this->config->get('ocfilter_consider_option')) {
            $where .= " OR EXISTS (SELECT pov.product_id FROM " . DB_PREFIX . "product_option_value pov WHERE pov.product_id = p.product_id AND pov.quantity > '0' AND COALESCE(IF(pov.rub_price = '-', p.rub_price - pov.rub_price, NULL), IF(pov.rub_price = '+', p.rub_price + pov.rub_price, NULL), IF(pov.rub_price = '*', p.rub_price + p.rub_price * pov.rub_price, NULL), IF(pov.rub_price = '%', p.rub_price + p.rub_price * (pov.rub_price / 100), NULL), IF(pov.rub_price = '=', pov.rub_price, NULL), p.rub_price + pov.rub_price, p.rub_price) BETWEEN '" . (float)$price_from . "' AND '" . (float)$price_to . "')";
          }
  ...
  {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_page op LEFT JOIN " . DB_PREFIX . "ocfilter_page_description opd ON (op.ocfilter_page_id = opd.ocfilter_page_id) WHERE op.status = '1' AND opd.language_id = '" . $this->config->get('config_language_id') . "' AND opd.city_id = '" . CITY_ID . "'");
  ...
  {
    $sql = "SELECT op.*, opd.title
          FROM " . DB_PREFIX . "ocfilter_page op 
            LEFT JOIN " . DB_PREFIX . "ocfilter_page_description opd ON (op.ocfilter_page_id = opd.ocfilter_page_id AND opd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND opd.city_id = '510')";
  ...
  public function getBrands()
  {
    $sql = "SELECT op.*, opd.title
          FROM " . DB_PREFIX . "ocfilter_page op 
            LEFT JOIN " . DB_PREFIX . "ocfilter_page_description opd ON (op.ocfilter_page_id = opd.ocfilter_page_id AND opd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND opd.city_id = '510')
            WHERE op.keyword LIKE 'brend/%'";
    $query = $this->db->query($sql);

    return $query->rows;
  }

  public function getBrandByKeyword($keyword)
  {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_page op 
            LEFT JOIN " . DB_PREFIX . "ocfilter_page_description opd ON (op.ocfilter_page_id = opd.ocfilter_page_id) 
            WHERE op.status = '1' AND op.keyword = '" . $this->db->escape($keyword) . "' AND opd.language_id = '" . $this->config->get('config_language_id') . "' AND opd.city_id = '" . CITY_ID . "' LIMIT 1");
    return $query->row;
  }
}
