...
class ModelCatalogCategory extends Model {
	...
	public function findSizes($parent_id = 0)
	{
		$query = $this->db->query("SELECT parent_id, sizes FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$parent_id . "'");

		if ($query->row) {
			if ($query->row['sizes']) {
				return $query->row['sizes'];
			} elseif ($query->row['parent_id']) {
				return $this->findSizes($query->row['parent_id']);
			}
		}
		return false;
	}
