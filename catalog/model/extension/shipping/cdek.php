...
class ModelExtensionShippingCdek extends Model {
		...
		if ($to_data && $address['city']) {
			...
			if (is_array($cdek_cities) && isset($cdek_cities['geonames']) && count($cdek_cities['geonames'])) {
				...
				if ($this->config->get('cdek_use_region_russia'))
				{
					...
					//$sql = "SELECT regionName FROM  `" . DB_PREFIX . "cdek_city` WHERE `cityName` = '".$this->db->escape($address['zone'])."'";
						...
						if ($status) {
							...
							if ($status) {
								...
								if ($this->config->get('cdek_work_mode') == 'single') {
									...
									$set_postomat = false;
									foreach ($exTariffList['postomat'] as $mtfkey => $mtfvalue)
									{
										if(!$set_postomat || (float)$mtfvalue['priceByCurrency'] < (float)$set_postomat['priceByCurrency'])
											$set_postomat = $mtfvalue;
									}
								...
								if (!empty($results)) {
									...
									if ($usePVZ) {
										...
										$postomat_list = $this->getPVZList($city_to, $weight, 'POSTOMAT');
									}
									...
									foreach ($postomat_list as $postomat_list_content) {
										$this->session->data['cdek']['postomatlist'] = $postomat_list_content['info'];
									}
	...
	private function getURL($url, response_parser $parser, $data = array()) {
		...
		if (!empty($data)) {
			file_put_contents(dirname(DIR_ROOT_PATH) . '/cdekcatalog.log', 'SDEK : ' . print_r($data, true), FILE_APPEND);
			if ($this->config->get('shipping_cdek_log')) {
	...
	public function getRegions($region_name = "", $page = 1) {
		$sql = "SELECT *, TRIM(`regionName`) as region FROM `" . DB_PREFIX . "cdek_city` WHERE TRIM(`regionName`) <> ''";
		if ($region_name) {
			$sql .= " AND `regionName` LIKE '%" . $this->db->escape($region_name) . "%'";
		}
		$total = $this->db->query($sql)->num_rows;
		$sql .= " GROUP BY `regionName` ORDER BY region LIMIT 10 OFFSET ". ((int)$page - 1) * 10;
		$query = $this->db->query($sql);

		return ['rows' => $query->rows, 'more' => $total > (int)$page * 10];
	}

	public function getCities($region_name = "", $city_name = "", $page = 0)
	{
		$sql = "SELECT *, TRIM(`cityName`) as city FROM `" . DB_PREFIX . "cdek_city` WHERE TRIM(`cityName`) <> '' ";
		if ($region_name) {
			$sql .= " AND `regionName` LIKE '%" . $this->db->escape($region_name) . "%' ";
		}
		if ($city_name) {
			// $sql .= " AND MATCH (cityName) AGAINST ('" . $this->db->escape($city_name) . "%') ";
			$sql .= " AND `cityName` LIKE '" . $this->db->escape($city_name) . "%' ";
		}
		$total = $this->db->query($sql)->num_rows;
		$sql .= " GROUP BY `cityName` ORDER BY city LIMIT 10 OFFSET ". ((int)$page - 1) * 10;
		$query = $this->db->query($sql);
