...
class CalculatePriceDeliveryCdek {
	...
	public function __construct($initiator = null) {
	     ...
	     if($initiator) {
	     	$this->jsonUrl = $initiator->getInfo()->getAjaxUrl().'calculator/calculate_price_by_json.php';
	     } else {
	     	$this->jsonUrl = 'https://api.cdek.ru/calculator/calculate_price_by_json.php';
	     }
	}
	...
	private function _getRemoteData($data) {
		...
		file_put_contents(dirname(DIR_ROOT_PATH) . '/cdeklog/'.$logname.'.log', 'url: ' . $this->jsonUrl .PHP_EOL, FILE_APPEND);
		file_put_contents(dirname(DIR_ROOT_PATH) . '/cdeklog/'.$logname.'.log','post: ' . $data_string .PHP_EOL, FILE_APPEND);
		curl_setopt($ch, CURLOPT_TIMEOUT_MS, 3000);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT_MS, 3000);

		$result = curl_exec($ch);
		file_put_contents(dirname(DIR_ROOT_PATH) . '/cdeklog/'.$logname.'.log','result: ' . $result .PHP_EOL, FILE_APPEND);
		curl_close($ch);
