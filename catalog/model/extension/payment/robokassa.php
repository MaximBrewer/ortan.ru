...
class ModelExtensionPaymentRobokassa extends Model {
	...
	public function sendSecondCheck($order_id)
    {
	    ...
	    /** @var array $fields */
		$fields = [
		  	'merchantId' => $this->config->get('robokassa_login'),
		    'id' => $order_id + 1,
		    'originId' => $order_id,
		    'operation' => 'sell',
		    'sno' => $this->config->get('robokassa_tax_type'),
		...
		if (!empty($shipping)) {
			...
			if ($shipping_price > 0) {
				...
				switch ($this->config->get('robokassa_tax'))
				{
					case "vat0":
						 $fields['vats'][] = ['type' => $this->config->get('robokassa_tax'), 'sum' => 0];
					case "none":
						$fields['vats'][] = ['type' => $this->config->get('robokassa_tax'), 'sum' => 0];
					break;
					...
					case "vat10":
						$fields['vats'][] = ['type' => $this->config->get('robokassa_tax'), 'sum' => ($price/100)*10];
					case "vat18":
						$fields['vats'][] = ['type' => $this->config->get('robokassa_tax'), 'sum' => ($price/100)*18];
					case "vat20":
						$fields['vats'][] = ['type' => $this->config->get('robokassa_tax'), 'sum' => ($price/100)*20];
					break;
				}
	    ...
	    $sign = $this->formatSignFinish(
	    	\base64_encode(
	    	    \md5(
	    	    	$startupHash .
			        ($this->config->get('robokassa_test') === 1 ? $this->config->get('robokassa_test_password_1') : $this->config->get('robokassa_password_1'))
		        )
		    )
	    );
