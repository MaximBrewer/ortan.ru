...
class cdek_integrator {
	...
	protected function getURL($url, response_parser $parser, $data = array()) {
		...
		if (!empty($data)) {
			...
			file_put_contents(dirname(DIR_ROOT_PATH) . '/cdek.log', 'SDEK url : ' . $url, FILE_APPEND);
			file_put_contents(dirname(DIR_ROOT_PATH) . '/cdek.log', 'SDEK data : ' . print_r($data, true), FILE_APPEND);
		}

		$out = curl_exec($ch);
		file_put_contents(dirname(DIR_ROOT_PATH) . '/cdek.log', 'SDEK out : ' . print_r($out, true), FILE_APPEND);
		curl_close($ch);
