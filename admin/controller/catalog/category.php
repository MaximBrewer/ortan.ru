...
class ControllerCatalogCategory extends Controller {
	...
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$url = isset($this->request->get['city_id']) ? '&city_id=' . $this->request->get['city_id'] : '';
	...
	protected function getList() {
		...
		$data['column_menu_name'] = $this->language->get('column_menu_name');
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['entry_menu_name'] = $this->language->get('entry_menu_name');
		...
		$data['entry_sort_type'] = $this->language->get('entry_sort_type');
		...
		foreach($data['cities'] as &$city){
			$city['link'] = $this->url->link('catalog/category/edit', 'token='.$this->session->data['token'].'&category_id='.$this->request->get['category_id'].'&city_id='.$city['city_id'], true);
		}
		$data['currentCity'] = $this->model_extension_module_citychooser_city->getCity(isset($this->request->get['city_id']) ? $this->request->get['city_id'] : 510);
		$data['_city_id'] = $data['currentCity']['city_id'];
		...
		if (isset($this->request->post['category_description'])) {
			...
			$data['category_description'] = $this->model_catalog_category->getCategoryDescriptions($this->request->get['category_id'], isset($this->request->get['city_id']) ? $this->request->get['city_id'] : 510);
		} else {
        ...
        $data['sort_types'] = $this->model_catalog_sort_type->getSorts();

        if (isset($this->request->post['sort_type'])) {
            $data['sort_type'] = $this->request->post['sort_type'];
        } elseif (!empty($category_info)) {
            $data['sort_type'] = $category_info['sort_type'];
        } else {
            $data['sort_type'] = '';
        }
		...
		if (isset($this->request->post['sizes'])) {
			...
			$data['sizes'] = $category_info['sizes'];
		} else {
			$data['sizes'] = '';
		}

		if (isset($this->request->post['sizes']) && is_file(DIR_IMAGE . $this->request->post['sizes'])) {
			$data['thumb_sizes'] = $this->model_tool_image->resize($this->request->post['sizes'], 100, 100);
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['sizes'])) {
			$data['thumb_sizes'] = $this->model_tool_image->resize($category_info['sizes'], 100, 100);
		} else {
			$data['thumb_sizes'] = $this->model_tool_image->resize('no_sizes.png', 100, 100);
		}
		...
		if (isset($this->request->post['category_template'])) {
			$data['category_template'] = $this->request->post['category_template'];
		} elseif (isset($this->request->get['category_id'])) {
			$data['category_template'] = $this->model_catalog_category->getCategoryTemplate($this->request->get['category_id']);
		} else {
			$data['category_template'] = array();
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['category_description'] as $language_id => $language) {
			foreach ($language as $city_id => $value) {
				if (false && (utf8_strlen($value['name']) < 2) || (utf8_strlen($value['name']) > 255)) {
					$this->error['name'][$language_id][$city_id] = $this->language->get('error_name');
				}
			}
		}
