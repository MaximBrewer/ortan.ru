...
class ControllerCatalogProduct extends Controller {
	...
	protected function validateReprice() {
	...
	public function reprice() {
		...
		if (isset($this->request->post['selected']) && $this->validateReprice()) {
			
			$this->load->model('localisation/currency');
			$curr = $this->model_localisation_currency->getCurrency(2);

			$price_rub = (float) str_replace([",", " "], [".", ""], $this->request->post['price_rub']);
			$price_cyn = (float) str_replace([",", " "], [".", ""], $this->request->post['price_cyn']);

			$price_rub || $price_rub = round($price_cyn / $curr['value']);
			$price_cyn || $price_cyn = round($price_rub * $curr['value']);

			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_catalog_product->setPriceProduct($product_id, $price_rub, $price_cyn, $curr['value']);
			}
	...
	protected function getList() {
		...
		$data['reprice'] = $this->url->link('catalog/product/reprice', 'token=' . $this->session->data['token'] . $url, true);
      ...
      $category =  $this->model_catalog_product->getProductCategories($result['product_id']);
		...
		$data['column_yprice'] = $this->language->get('column_yprice');
		...
		$data['entry_yprice'] = $this->language->get('entry_yprice');
        ...
        $data['tab_yandexmarket'] = $this->language->get('tab_yandexmarket');
        $data['entry_isyandexlmarket'] = $this->language->get('entry_isyandexlmarket');
        $data['entry_name_ym'] = $this->language->get('entry_name_ym');
	...
	$data['currencies'] = $this->model_localisation_currency->getCurrencies();
        ...
        $data['tab_yandexmarket'] = $this->language->get('tab_yandexmarket');
        $data['entry_isyandexmarket'] = $this->language->get('entry_isyandexmarket');
        $data['entry_name_ym'] = $this->language->get('entry_name_ym');
		...
		if (isset($this->request->post['yprice'])) {
			$data['yprice'] = $this->request->post['yprice'];
		} elseif (!empty($product_info)) {
			$data['yprice'] = $product_info['yprice'];
		} else {
			$data['yprice'] = '';
		}
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
        ...
        if (isset($this->request->post['store'])) {
            ...
            $data['store'] = $this->model_catalog_product->getProductStore($product_info['product_id']);
        } else {
            $data['store'] = '1';
        }

        if (isset($this->request->post['isyandexmarket'])) {
            $data['isyandexmarket'] = $this->request->post['isyandexmarket'];
        } elseif (!empty($product_info)) {
            $data['isyandexmarket'] = $product_info['isyandexmarket'];
        } else {
            $data['isyandexmarket'] = '0';
        }
