...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
        ...
        $this->document->addStyle('view/javascript/select2/select2.min.css');
		...
		foreach ($results as $result) {
			...
			//$histories = $this->model_sale_order->getOrderHistoriesDesc($result['order_id']);
			$histories = $this->model_sale_order->getOrderHistoriesFilledDesc($result['order_id']);
			$result['histories'] = [];
			foreach ($histories as $history) {
			    $user_name = $history['firstname'];
			    if ($history['lastname'] and $history['firstname'] != $history['lastname']) {
			        $user_name .= ($user_name ? ' ' : '') . $history['lastname'];
                }
			...
			$data['orders'][] = array(
				...
				'email'      => $result['email'],
				'comment'      => preg_replace('~(.*)\[CDEK\].*\[/CDEK\]~ui', '$1', $result['comment']),
        ...
        $data['entry_client_comment'] = $this->language->get('entry_client_comment');
        ...
        $data['filter_order_status'] = $filter_order_status !== null ? explode(',', $filter_order_status) : [];
        ...
        $data['order_statuses'] = array_merge(['' => ['name'=>$this->language->get('text_missing')]], $data['order_statuses']);
	...
	public function info() {
		...
		if ($order_info) {
			...
			if ($order_info['payment_code'] && $this->user->hasPermission('access', 'extension/payment/' . $order_info['payment_code'])) {
				if (is_file(DIR_APPLICATION . 'controller/extension/payment/' . $order_info['payment_code'] . '.php')) {
				...
				foreach ($products as $product) {
					...
					if ($product_info) {
						...
						$product_data[] = array(
							...
							'weight'   => $this->weight->format(((float)$product_info['weight'] + (float)$option_weight) * (int)$product['quantity'], $product_info['weight_class_id'], $this->language->get('decimal_point'), $this->language->get('thousand_point'))
						);
					}
				}
