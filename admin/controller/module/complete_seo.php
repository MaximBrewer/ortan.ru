  ...
  public function generator_category($mode, $simulate, $empty_only, $redirect) {
    ...
    foreach($this->request->post['langs'] as $lang)
    {
      ...
      foreach ($rows as $row) {
        ...
        $value = str_replace('[current]', $row[$field], $pattern);
        $value = $this->model_tool_seo_package->transformCategory($value, $lang, $row, $this->store, isset($this->request->get['city']) ? $this->request->get['city'] : 510);
...
Sitemap: '.HTTP_CATALOG.'sitemap.xml';
  ...
  public function get_value() {
    ...
    foreach ((array) $fields as $field) {
      ...
      if ($type == 'category') {
        ...
        if ($store_id && !empty($row['seo_'.$type.'_description'][$store_id])) {
          if (isset($row[$type.'_description'])) {
            foreach ($row[$type.'_description'] as $language_id => $language) {
              foreach ($language as $city_id => $desc) {
              $row[$type.'_description'][$language_id][$city_id]['orig_name'] = $row[$type.'_description'][$language_id][$city_id]['name'];
              $row[$type.'_description'][$language_id][$city_id]['orig_description'] = $row[$type.'_description'][$language_id][$city_id]['description'];
              $row[$type.'_description'][$language_id][$city_id] = array_merge($row[$type.'_description'][$language_id][$city_id], array_filter($row['seo_'.$type.'_description'][$store_id][$language_id][$city_id]));
            }
          }
        }
      }
      ...
      if ($type == 'category') {
        $pattern = str_replace('[current]', $row[$type.'_description'][$lang][$this->request->get['city']][$field], $pattern);
      } else {
      ...
      $this->load->model('tool/seo_package');
      if ($type == 'category') {
        $value = $this->model_tool_seo_package->{'transform'.ucfirst($type)}($pattern, $lang, $row, $store_id, true, isset($this->request->get['city']) ? $this->request->get['city'] : 510);
      }else{
      ...
      if ($store || $type == 'category') {
        $values[$type.'_description['.$lang.']['.(isset($this->request->get['city']) ? $this->request->get['city'] : 510).']['.$field.']'] = $value;
      } elseif ($store || $type == 'manufacturer') {
    ...
    if ($type == 'image') {
      ...
      if (in_array($type, array('product', 'category', 'manufacturer'))) {
        ...
        if (in_array($type, array('category'))) {
          $columns[] = array( 'db' => 'city_id', 'dt' => $dt++, 'formatter' => 'get_city', 'table_alias' => 'd');
        }
      }
  ...
  public function editor_update() {
    ...
    $city_id = isset($this->request->post['city']) ? $this->request->post['city'] : 510;
    ...
    if (in_array($type, array('product', 'category', 'information', 'manufacturer', 'common', 'special', 'redirect', 'absolute'))) {
      ...
      if ($col == 'seo_keyword') {
        ...
        // update the value
        if($type == 'category'){
          $this->db->query("UPDATE " . DB_PREFIX . $seoDesc . $type . "_description SET " . $col . " = '" . $this->db->escape($value) . "' WHERE " . $primaryKey . " = '" . (int)$pk . "' AND language_id = '" . (int)$lang . "' AND city_id = '" . (int)$city_id . "'" . $extraWhere);
        } else {
