...
class ControllerLocalisationCurrency extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$post['value'] = 1 / $post['value'];
			$this->model_localisation_currency->addCurrency($post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$post = $this->request->post;
			$post['value'] = 1 / $post['value'];
			$this->model_localisation_currency->editCurrency($this->request->get['currency_id'], $post);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['value'])) {
			...
			$data['value'] = round(1 / $currency_info['value'], 2);
		} else {
			$data['value'] = '';
		}
