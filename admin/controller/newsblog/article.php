...
class ControllerNewsBlogArticle extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (isset($this->request->post['author_image'])) {
			...
			$data['author_image'] = $article_info['author_image'];
		} else {
			$data['author_image'] = '';
		}
		...
		} elseif (!empty($article_info) && is_file(DIR_IMAGE . $article_info['author_image'])) {
			$data['author_thumb'] = $this->model_tool_image->resize($article_info['author_image'], 100, 100);
		} else {
			$data['author_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		$data['author_placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
