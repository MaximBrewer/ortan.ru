<?php
class ModelCatalogOCFilterPage extends Model {
	public function addPage($data, $city_id = 510) {
        ...
        if (isset($data['image'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_page SET image = '" . $this->db->escape($data['image']) . "' WHERE ocfilter_page_id = '" . (int)$ocfilter_page_id . "'");
        }

		foreach ($data['page_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_page_description SET ocfilter_page_id = '" . (int)$ocfilter_page_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', city_id ='". $this->db->escape($city_id) ."'");
		}
	...
	public function editPage($ocfilter_page_id, $data, $city_id = 510) {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "ocfilter_page_description WHERE ocfilter_page_id = '" . (int)$ocfilter_page_id . "' AND `city_id` = '".$city_id."'");
	...
	public function getTotalPages() {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "ocfilter_page op LEFT JOIN " . DB_PREFIX . "ocfilter_page_description opd ON (op.ocfilter_page_id = opd.ocfilter_page_id) WHERE opd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND opd.city_id = '510'";
	...
	public function getPageDescriptions($ocfilter_page_id, $city_id = 510) {
		$page_description_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_page_description WHERE ocfilter_page_id = '" . (int)$ocfilter_page_id . "' AND city_id = '".$city_id."'");
