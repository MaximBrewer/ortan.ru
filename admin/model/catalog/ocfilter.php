...
class ModelCatalogOCFilter extends Model {
  ...
  public function getOptionCategories($option_id) {
    ...
    $query = $this->db->query("SELECT oo2c.category_id, cd.name FROM " . DB_PREFIX . "ocfilter_option_to_category oo2c LEFT JOIN " . DB_PREFIX . "category_description cd ON (oo2c.category_id = cd.category_id) WHERE oo2c.option_id = '" . (int)$option_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'AND cd.city_id = '" . 510 . "'");
  ...
  private function getCategoriesByParentId($parent_id = 0) { # For options list and form
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd.city_id = '" . (int)510 . "' ORDER BY c.sort_order, cd.name ASC");
