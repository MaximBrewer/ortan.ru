...
class ModelToolSeoPackage extends Model {
  ...
  public function getGrandParentCategoryName($category_id, $lang, $city = 510) {
		$query = $this->db->query("SELECT cd.name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category ch ON (c.parent_id = ch.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd ON (cd.category_id = ch.parent_id) WHERE c.category_id = '" . (int)$category_id . "' AND cd.language_id = " . $lang . " AND cd.city_id = '" . $city . "' LIMIT 1")->row;
  ...
  public function getParentCategoryName($category_id, $lang, $city = 510) {
		...
		$query = $this->db->query("SELECT cd.name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON c.parent_id = cd.category_id WHERE c.category_id = '" . (int)$category_id . "' AND cd.language_id = " . $lang . " AND cd.city_id = '" . $city . "' LIMIT 1")->row;
  ...
  public function getChildCategoryName($category_id, $lang, $city = 510) {
		//$query = $this->db->query("SELECT cd.name FROM " . DB_PREFIX . "category_description cd LEFT JOIN " . DB_PREFIX . "category c ON c.category_id = cd.category_id WHERE cd.category_id = '" . (int)$category_id . "' AND language_id = " . $lang . " LIMIT 1")->row;
		$query = $this->db->query("SELECT cd.name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON c.category_id = cd.category_id WHERE c.parent_id = '" . (int)$category_id . "' AND cd.language_id = " . $lang . " AND cd.city_id = '" . $city . "' LIMIT 1")->row;
  ...
  public function getManufacturerData($manufacturer_id, $lang, $store_id) {
	...
	public function commonTransform($pattern, $lang, $row, $store_id = 0, $get_value = false, $city = 510) {
    $replace  = array();
    ...
    */
	...
	public function transformCategory($pattern, $lang, $row, $store_id = 0, $get_value = false, $city = 510) {
    $pattern = $this->commonTransform($pattern, $lang, $row, $store_id, $get_value, $city);
		if (!isset($row['name'])) {
      if (isset($row['orig_name'])) {
        $row['name'] = $row['orig_name'];
      } else {
   			$row['name'] = isset($row['category_description'][$lang][$city]['name']) ? $row['category_description'][$lang][$city]['name'] : '';
      }
		}
    ...
    if (!isset($row['description'])) {
      if (isset($row['orig_description'])) {
        $row['description'] = $row['orig_description'];
   			...
   			$row['description'] = isset($row['category_description'][$lang][$city]['description']) ? $row['category_description'][$lang][$city]['description'] : '';
      }
		}
    ...
    if (strpos($pattern, '[grandparent]') !== false)
			$replace['[grandparent]'] = !empty($row['category_id']) ? trim($this->getGrandParentCategoryName($row['category_id'], $lang, $city)) : '';
    if (strpos($pattern, '[parent]') !== false)
			$replace['[parent]'] = !empty($row['category_id']) ? trim($this->getParentCategoryName($row['category_id'], $lang, $city)) : '';
    if (strpos($pattern, '[child]') !== false)
			$replace['[child]'] = !empty($row['category_id']) ? trim($this->getChildCategoryName($row['category_id'], $lang, $city)) : '';
