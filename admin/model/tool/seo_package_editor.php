            ...
            foreach ($related as $rel) {
          	...
          	$data[$i]['related']['rows'] = $related;
				...
				if ( isset( $column['formatter'] ) ) {
					if($column['formatter'] == 'get_city'){
						$this->load->model('extension/module/citychooser/city');
						$city = $this->model_extension_module_citychooser_city->getCity($data[$i][ $column['db'] ]);
						$row[ $column['dt'] ] = '<span data-city='.$city['city_id'].'>'.$city['city'].'</span>';
					} else {
