...
class ModelNewsBlogArticle extends Model {
	public function addArticle($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "newsblog_article SET date_available = '" . $this->db->escape($data['date_available']) . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', author = '" . $data['author'] . "', date_added = NOW()");
		...
		if (isset($data['author_image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "newsblog_article SET author_image = '" . $this->db->escape($data['author_image']) . "' WHERE article_id = '" . (int)$article_id . "'");
		}

		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "newsblog_article_description SET article_id = '" . (int)$article_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', preview = '" . $this->db->escape($value['preview']) . "', description = '" . $this->db->escape($value['description']) . "', description2 = '" . $this->db->escape($value['description2']) . "', list = '" . $this->db->escape($value['list']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
	...
	public function editArticle($article_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "newsblog_article SET date_available = '" . $this->db->escape($data['date_available']) . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', author = '" . $data['author'] . "', date_modified = NOW() WHERE article_id = '" . (int)$article_id . "'");
