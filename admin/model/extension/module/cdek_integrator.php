...
class ModelExtensionModuleCdekIntegrator extends Model {
	...
	public function statusApi($order_id, $status_id, $notify, $comment) {
		...
		if(!$token) {
			$this->log->write('CDEK integration: ошибка входа по api!'); return;
		} 
	...
	public function addCities($data, $country) {
		if (!empty($data) && count($data) >= 1) {
			$sql = "INSERT INTO " . DB_PREFIX . "cdek_city (id, name, cityName, regionName, center, cache_limit, country) VALUES ";
			$i = 1;
			foreach ($data as $value) {
				if ($i != 1000) {
					if (!next($data)) {
						$sql .= "('" . (int)$value['id'] . "', '" . $this->db->escape($value['name']) . "', '" . $this->db->escape($value['cityName']) . "', '" . $this->db->escape($value['regionName']) . "', '0', '0', '".$country."');";
					} else {
						$sql .= "('" . (int)$value['id'] . "', '" . $this->db->escape($value['name']) . "', '" . $this->db->escape($value['cityName']) . "', '" . $this->db->escape($value['regionName']) . "', '0', '0', '".$country."'), ";
					}
				} else {
					if (!next($data)) {
						...
						$sql .= "('" . (int)$value['id'] . "', '" . $this->db->escape($value['name']) . "', '" . $this->db->escape($value['cityName']) . "', '" . $this->db->escape($value['regionName']) . "', '0', '0', '".$country."');";
						$this->db->query($sql);
						$sql = "INSERT INTO " . DB_PREFIX . "cdek_city (id, name, cityName, regionName, center, cache_limit) VALUES ";
						$i = 0;
					}
				}
				$i = $i + 1;
